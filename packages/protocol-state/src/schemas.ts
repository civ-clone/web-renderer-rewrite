...
const stableEntityId = z.string().min(1);
const rngMetadataSchema = z.object({
  algorithm: z.string().min(1),
  seed: z.string().min(1),
  counter: z.number().int().nonnegative().optional(),
});
const rngTraceSchema = z.object({
...
const mandatoryRequirementSchema = z.object({
  ...
  valueStableId: stableEntityId.optional(),
});
...
export const unitActionDescriptorSchema = z.object({
  ...
  unitStableId: stableEntityId.optional(),
